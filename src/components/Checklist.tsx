...
const Checklist = ({
  ...
  if (!Array.isArray(items)) {
    throw new Error(
      `Failed to render Checklist: Invalid items type ${typeof items}`
    );
  }

  return (
    <ul
      className={`${checkable ? "cursor-pointer" : ""} list-none flex flex-col gap-2 p-0`}
      data-is-checkable={checkable}
    >
