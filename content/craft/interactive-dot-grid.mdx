...
date: 2026-03-07
description: "Play with a canvas of dots that grow and shrink as you move your cursor."
...
Now things get interesting. The browser tells us where the cursor is in screen pixels (clientX, clientY), but our dots live in the SVG — which could be scaled, shifted, or stretched. So we need to translate between the two. Luckily, SVG has a built-in trick: getScreenCTM() gives you the transformation matrix from SVG space to screen space. Flip it around (invert it), and now you can go from screen coordinates back to SVG coordinates.
