...
Once you can answer that question, you can map the result to any CSS property—blur, scale, opacity, hue, rotation. The image above uses blur and scale. The [ripple button](/craft/ripple-button) in the next post uses click position. Same math, different costume.
...
The API surface is intentionally small. You get back a coordinate and a bounding box. What you _do_ with the distance—whether you map it to blur, scale, opacity, hue rotation, or something else entirely—is up to you. The hook doesn't care. It just answers that original question: _how far?_
